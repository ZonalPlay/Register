...
use pocketmine\event\player\PlayerInteractEvent; 
	...
	class Main extends PluginBase implements Listener {
		...
		public function onPlayerJoin(PlayerJoinEvent $event) {
			...
			if(isset($user["nickname"])) {
				...
				if($ip == $user["ipLast"]) {
					...
					$player->sendMessage(" §e§lАвторизация §a× §f Вы уже вводили свой пароль! §aПриятной игры!");
				} else
					$player->sendMessage(" §e§lАвторизация §a× §f Введите свой пароль в чат, который вводили при §cрегистрации, §fили зайдите в настройки и смините ник на новый. ");
			} else $player->sendMessage(" §c§lРегистрация §a× §f Придумайте и §aвведите свой пароль §fв чат, для§c регистрации.");
			$sql->finalize();
		}
		...
		public function onPlayerCommandPreprocess(PlayerCommandPreprocessEvent $event) {
			$login = " §a§lАвторизация §a× §f Введите свой §aпароль §fв чат, который вводили при §cрегистрации, или зайдите в §aнастройки и смените свой §cник";
			$player = $event->getPlayer();
						...
						if($sql instanceof \SQLite3Result) {
							...
							if(!empty($user["nickname"])) {
								if($pass == $user["password"]) {
									...
									$player->sendMessage(" §e§lАвторизация §a× §f Вы успешно §aавторизировались §fна сервере. Приятной игры!");
								} else $player->sendMessage(" §e§lАвторизация §a× §f Вы ввели §cневерный пароль.");
							} else {
								if(!isset($this->reg[$name])) {
									$this->reg[$name] = $pass;
									$player->sendMessage(" §c§lРегистрация §a× §f Введите §c<свой пароль> §fв чат §aповторно!");
								} else {
									if($pass == $this->reg[$name]) {
										...
										$player->sendMessage(" §c§lРегистрация §a× §f Вы успешно §aзарегистрировались, §fне забудьте свой §cпароль!");
									} else {
										unset($this->reg[$name]);
										$player->sendMessage(" §a§lАвторизация§a× §f Вы ввели §cневерный пароль.");
									}
								}
							}
						}
